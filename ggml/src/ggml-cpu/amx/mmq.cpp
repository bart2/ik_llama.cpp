...
// Compatibility macros for ik_llama.cpp
...
// Syscall constants for AMX permission
...
void ggml_amx_mul_mat(
    ...
    fprintf(stderr, "[AMX] Executing AMX matmul: [%ld, %ld] x [%ld, %ld] (type: %s)\n",
...
// Helper functions for AMX integration
...
#else
    // Assume AMX is available on other platforms
    return true;
#endif
}
...
bool ggml_amx_is_available(void) {
    ...
    if (available < 0) {
        available = ggml_amx_init() ? 1 : 0;
        if (available) {
            fprintf(stderr, "[AMX] AMX hardware initialized successfully\n");
        } else {
            fprintf(stderr, "[AMX] AMX hardware initialization failed\n");
        }
    }
    return available == 1;
}
...
bool ggml_amx_supports_type(enum ggml_type type) {
    return qtype_has_amx_kernels(type);
}

// Stub for ggml_barrier - ik_llama.cpp doesn't use threadpools
void ggml_barrier(struct ggml_threadpool * tp) {
    (void)tp; // Unused
    // No-op for ik_llama.cpp which doesn't have threadpool support
}
