...
class tensor_traits : public ggml::cpu::tensor_traits {
    ...
    bool compute_forward(struct ggml_compute_params * params, struct ggml_tensor * op) override {
        if (op->op == GGML_OP_MUL_MAT) {
            fprintf(stderr, "[AMX] Using AMX acceleration for MUL_MAT operation (%s)\n", op->name);
...
ggml_backend_buffer_type_t ggml_backend_amx_buffer_type() {
    ...
    if (!ggml_amx_init()) {
        fprintf(stderr, "AMX initialization failed - AMX not available\n");
        return nullptr;
    }
    fprintf(stderr, "AMX initialized successfully - AMX acceleration enabled\n");
